...
function submitReply(topicId, form) {
    ...
    if (hasSorunsal) {
        ...
        fetch('/api/chat', {
            ...
            body: JSON.stringify({
                message: "Konu: " + topic.topic + "\nDetay: " + topic.detail + "\nKullanıcı diyor ki: " + text
            })
        })
        .then(function(res) { return res.json(); })
        .then(function(data) {
            var freshTopics = getAdviceTopics();
            var freshTopic = freshTopics.find(function(t) { return t.id === currentTopicId; });
            if (freshTopic) {
                ...
                saveAdviceTopics(freshTopics);
                renderAdviceTopics();
                setTimeout(function() {
                    var repliesSection = document.getElementById("replies-" + currentTopicId);
                    if (repliesSection) repliesSection.classList.add("open");
                    var toggleBtn = document.getElementById("toggle-" + currentTopicId);
        ...
        .catch(function() {
            isSubmitting = false;
            showToast("Bot şu an müsait değil, tekrar dene.");
        });
    } else {
        setTimeout(function() {
